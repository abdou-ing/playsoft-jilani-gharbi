...
// Retrieve root groups and all descendants
     ...
     .then(function rootGroupsRetrieved(rootConnectionGroups) {
        ...
        // AUTO-SWITCH PATCH: If user has multiple connections but only opened
        // one, automatically add all other connections to the current view
        // so that the native connection switcher is available immediately.
        if (clientPages.length > 1) {

            // Get the current set of client IDs from the URL
            var currentIds = ManagedClientGroup.getClientIdentifiers($routeParams.id);

            // Only auto-add if user navigated to a single connection
            if (currentIds.length === 1) {

                // Collect all connection IDs from the connection tree
                var allIds = [];
                clientPages.forEach(function(page) {
                    if (page.url && page.url.indexOf('/client/') === 0) {
                        var pageClientId = page.url.substring('/client/'.length);
                        if (allIds.indexOf(pageClientId) === -1) {
                            allIds.push(pageClientId);
                        }
                    }
                });

                // If there are additional connections the user has access to,
                // redirect to include all of them
                if (allIds.length > 1) {

                    // Ensure the originally-requested connection is first
                    var orderedIds = [currentIds[0]];
                    ...
                    // Redirect to multi-connection URL
                    $location.path('/client/' + ManagedClientGroup.getIdentifier(orderedIds));
                }
            }
        }
