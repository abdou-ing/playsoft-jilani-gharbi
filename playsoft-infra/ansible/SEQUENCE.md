...
sequenceDiagram
    ...
    rect rgb(255, 255, 255)
        ...
        rect rgb(255, 245, 220)
            ...
            alt connection_type = ssh
                loop for each SSH VM
                    AC->>PVE: iptables NAT PREROUTING port (4000 + vmid-400) to vm_ip:22
                    AC->>PVE: iptables FORWARD ACCEPT to vm_ip:22
                end
                ...
                loop for each VNC VM
                    ...
                    AC->>PVE: qm stop and qm start vmid
                end
            else connection_type = win
                loop for each Windows VM
                    AC->>PVE: evil-winrm — Set network Private + enable RDP firewall rule
                    AC->>PVE: iptables NAT PREROUTING port (3310 + index) to vm_ip:3389
                    AC->>PVE: iptables FORWARD ACCEPT to vm_ip:3389
                end
            end
        end
        ...
        rect rgb(255, 230, 230)
            ...
            AC->>AC: build guac_connections list (vnc / ssh / rdp per vm)
            AC->>AC: build guacamole_users list (USER_PREFIX+N mapped to connection)

            loop for each connection
                AC->>GUAC: POST /api/connections (scicore.guacamole collection)
...
sequenceDiagram
    ...
    rect rgb(255, 255, 255)
        Op->>AC: ansible-playbook cleanup_ssh.yml OR cleanup_vnc.yml OR cleanup_win.yml
