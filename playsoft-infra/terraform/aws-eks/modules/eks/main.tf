############################################
# IAM roles, referenced by name only.
############################################
...
resource "aws_instance" "bastion" {
  ...
  vpc_security_group_ids = [aws_security_group.node_ssh[0].id]
...
resource "aws_eks_addon" "ebs_csi" {
  ...
  depends_on = [aws_eks_node_group.default, aws_eks_pod_identity_association.ebs_csi]
}
...
############################################
# EKS Pod Identity associations -- no OIDC provider, no :sub/:aud trust
############################################
...
resource "aws_eks_pod_identity_association" "lb_controller" {
  count = var.lb_controller_role_arn != "" ? 1 : 0
