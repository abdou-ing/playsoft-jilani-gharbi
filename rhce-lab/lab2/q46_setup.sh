...
case "$lang" in
  en)
    question="Since Ansible 2.10, the Ansible package was split. Which package name should you install to get the core Ansible engine and the \`ansible\` command?"
    ...
    instructions='[{"instruction": "To install the core Ansible engine, use", "command": "pip install ansible-core"}]'
    ...
    ;;
  fr)
    question="Depuis Ansible 2.10, le paquet Ansible a été divisé. Quel nom de paquet faut-il installer pour obtenir le moteur Ansible et la commande \`ansible\` ?"
    ...
    instructions='[{"instruction": "Pour installer le moteur Ansible, utilisez", "command": "pip install ansible-core"}]'
    answer_a="ansible-core"
    answer_b="ansible"
    answer_c="ansible-base"
    answer_d="ansible-engine"
