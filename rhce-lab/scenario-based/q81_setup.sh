...
case "$lang" in
  en)
    ...
    inst1="Write the playbook at \`$pb_path\` using the \`command\` module to run \`chage\` on each webserver:"
  ...
  fr)
    ...
    inst1="Écrivez le playbook à \`$pb_path\` avec le module \`command\` pour exécuter \`chage\` sur chaque webserver :"
