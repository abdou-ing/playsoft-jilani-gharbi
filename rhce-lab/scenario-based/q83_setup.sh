...
case "$lang" in
  en)
    ...
    inst1="Write the playbook at \`$pb_path\` using the \`command\` module to set the account expiry with \`chage -E\`:"
  ...
  fr)
    ...
    inst1="Écrivez le playbook à \`$pb_path\` avec le module \`command\` pour définir l'expiration du compte avec \`chage -E\` :"
