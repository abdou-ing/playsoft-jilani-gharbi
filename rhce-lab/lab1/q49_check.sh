...
declare -A messages_en=(
  ...
  ["no_file"]="Inventory file $inventory_path does not exist. Create it with [webservers1] containing web1 and [webservers2] containing web2."
  ["no_web1"]="web1 is not listed under [webservers1] in $inventory_path."
  ["no_web2"]="web2 is not listed under [webservers2] in $inventory_path."
  ["wrong_group_web1"]="web1 must be under [webservers1], not another group."
  ["wrong_group_web2"]="web2 must be under [webservers2], not another group."
)
declare -A messages_fr=(
  ...
  ["no_file"]="Le fichier d'inventaire $inventory_path n'existe pas. Créez-le avec [webservers1] contenant web1 et [webservers2] contenant web2."
  ["no_web1"]="web1 n'est pas listé sous [webservers1] dans $inventory_path."
  ["no_web2"]="web2 n'est pas listé sous [webservers2] dans $inventory_path."
  ["wrong_group_web1"]="web1 doit être sous [webservers1], pas dans un autre groupe."
  ["wrong_group_web2"]="web2 doit être sous [webservers2], pas dans un autre groupe."
)
...
# CHECK 4 — web1 must be in the [webservers1] group specifically
# (catches: candidate put web1 under [webservers2] or a custom group name)
web1_group=$(get_host_group web1)
if [[ "$web1_group" != "webservers1" ]]; then
  echo "$(get_message wrong_group_web1)"; exit 0
fi

# CHECK 5 — web2 must be in the [webservers2] group specifically
# (catches: candidate put web2 under [webservers1] or a custom group name)
web2_group=$(get_host_group web2)
if [[ "$web2_group" != "webservers2" ]]; then
